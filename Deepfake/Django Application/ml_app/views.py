...
def detect_deepfake(request):
    ...
    # Check if video format is allowed
    ...
    video_path = os.path.join(settings.PROJECT_DIR, 'uploaded_videos', saved_video_file)
...
def predict_page(request):
    if request.method == "GET":
        ...
        model = Model(2)  # Create model instance first
        model_name = get_accurate_model(sequence_length)  # Get the model filename
        path_to_model = model_name  # get_accurate_model already returns full path
